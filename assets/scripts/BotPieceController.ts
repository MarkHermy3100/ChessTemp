        ...
        if (Math.abs(this.pieceName) == 1) {
            ...
            && BoardState.convertCoordinates(this.piecePosition)[1] == BoardState.justPassed[1]) {
                ...
                if (BoardState.isInCheck()) {
                    if (this.pieceName == -1) {
                        BoardState.checkPosition.x = BoardState.convertUIPosition(BoardState.blackKing).x;
                        BoardState.checkPosition.y = BoardState.convertUIPosition(BoardState.blackKing).y;
                    }
                    else {
                        BoardState.checkPosition.x = BoardState.convertUIPosition(BoardState.whiteKing).x;
                        BoardState.checkPosition.y = BoardState.convertUIPosition(BoardState.whiteKing).y;
                    }
                }
                return;
            }
